      ...
      {itinerary && (
        <div className="bg-blue-900 rounded-xl shadow-2xl p-8 max-w-lg w-full flex flex-col items-center border border-blue-700">
          ...
          <div className="flex flex-wrap gap-4 mb-4 w-full justify-center">
            <div className="flex items-center gap-2 bg-blue-800 rounded px-3 py-1 text-blue-200">
          ...
          <div className="w-full">
            <h3 className="text-xl font-semibold text-blue-200 mb-2 mt-2">Itinerary</h3>
            <div className="flex flex-col gap-4">
              {(itinerary.program ?? []).map((day, i) => (
                <div key={i} className="bg-blue-800 rounded-lg p-4 shadow flex flex-col gap-2 border border-blue-700">
                  <div className="flex items-center gap-2 mb-2">
                    <span role="img" aria-label="calendar">📅</span>
                    <span className="text-lg font-bold text-white">Day {day?.day ?? i + 1}</span>
                  </div>
                  <ul className="flex flex-col gap-2">
                    {(day?.activities ?? []).map((activity, j) => (
                      <li key={j} className="bg-blue-950 rounded p-3 flex flex-col gap-1 border border-blue-700">
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-blue-100">{activity?.name ?? "Activity"}</span>
                          <span className="text-xs text-blue-400">({activity?.time ?? ""})</span>
                        </div>
                        <div className="text-blue-200 text-sm">{activity?.description ?? ""}</div>
